...
$senderResponse = $api->sender(array('Limit' => '-1'));
if(isset($api->_response_code) && $api->_response_code === MailjetAPI::MAILJET_STATUS_CODE_OK_GET){ 
	$senders = array();
...
if($submitted) {
	...
	if(empty($errors)) {
		...
		elseif (isset($api->_response_code) && $api->_response_code === MailjetAPI::MAILJET_STATUS_CODE_OK_ERROR) {
		 	$msg = 'Erreur pendant la création de la campagne. ';
		 	$errors[] = $msg;
		}
		else {
			$errors[] = 'Erreur interne pendant la création de la campagne.';
		}
	}
}
else {
	...
	<body>
		...
		<div id="content">
			...
			<div id="breadcrumb">
				...
				<a href="#" class="current">Création d'une campagne</a>
			</div>
				...
				<div class="col-md-7">
					...
					<?php if ($created): ?>
						<div class="alert alert-success">
							<strong>La campagne a été créée.</strong> <a href="view.php?id=<?php echo $id ?>">Voir la campagne</a>
						</div>
					<?php else: ?>
